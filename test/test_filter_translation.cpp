...
void PushFilter(TableFilterSet &filter_set, idx_t column_index, unique_ptr<TableFilter> filter,
                const LogicalType &column_type) {
	filter_set.PushFilter(ProjectionIndex(column_index), ExpressionFilter::FromTableFilter(*filter, column_type));
}

} // namespace

TEST_CASE("equality on integer", "[filter_translation]") {
	LegacyConstantFilter filter(ExpressionType::COMPARE_EQUAL, Value::INTEGER(42));
...
TEST_CASE("not-equal on integer", "[filter_translation]") {
	LegacyConstantFilter filter(ExpressionType::COMPARE_NOTEQUAL, Value::INTEGER(42));
	auto expr = TranslateOrWidenFilter(filter, MakeField("col"));
	REQUIRE(expr->ToString() == "ref(name=\"col\") != 42");
}

TEST_CASE("less-than on integer", "[filter_translation]") {
	LegacyConstantFilter filter(ExpressionType::COMPARE_LESSTHAN, Value::INTEGER(10));
	auto expr = TranslateOrWidenFilter(filter, MakeField("col"));
	REQUIRE(expr->ToString() == "ref(name=\"col\") < 10");
}

TEST_CASE("less-than-or-equal on integer", "[filter_translation]") {
	LegacyConstantFilter filter(ExpressionType::COMPARE_LESSTHANOREQUALTO, Value::INTEGER(10));
	auto expr = TranslateOrWidenFilter(filter, MakeField("col"));
	REQUIRE(expr->ToString() == "ref(name=\"col\") <= 10");
}

TEST_CASE("greater-than on integer", "[filter_translation]") {
	LegacyConstantFilter filter(ExpressionType::COMPARE_GREATERTHAN, Value::INTEGER(99));
	auto expr = TranslateOrWidenFilter(filter, MakeField("col"));
	REQUIRE(expr->ToString() == "ref(name=\"col\") > 99");
}

TEST_CASE("greater-than-or-equal on string", "[filter_translation]") {
	LegacyConstantFilter filter(ExpressionType::COMPARE_GREATERTHANOREQUALTO, Value("abc"));
...
TEST_CASE("field name comes from SchemaField", "[filter_translation]") {
	LegacyConstantFilter filter(ExpressionType::COMPARE_EQUAL, Value("hello"));
...
TEST_CASE("date equality", "[filter_translation]") {
	...
	LegacyConstantFilter filter(ExpressionType::COMPARE_EQUAL, Value::DATE(2024, 1, 1));
...
TEST_CASE("time greater-than", "[filter_translation]") {
	...
	LegacyConstantFilter filter(ExpressionType::COMPARE_GREATERTHAN, Value::TIME(12, 0, 0, 0));
...
TEST_CASE("timestamp less-than", "[filter_translation]") {
	...
	LegacyConstantFilter filter(ExpressionType::COMPARE_LESSTHAN, Value::TIMESTAMP(2024, 1, 1, 0, 0, 0, 0));
...
TEST_CASE("timestamp_tz equality", "[filter_translation]") {
	LegacyConstantFilter filter(ExpressionType::COMPARE_EQUAL, Value::TIMESTAMPTZ(timestamp_tz_t(1704067200000000LL)));
...
TEST_CASE("IN with integers", "[filter_translation]") {
	...
	LegacyInFilter filter(std::move(values));
	auto expr = TranslateOrWidenFilter(filter, MakeField("col"));
...
TEST_CASE("unconvertible comparison value widens to AlwaysTrue", "[filter_translation]") {
	LegacyConstantFilter filter(ExpressionType::COMPARE_EQUAL, Value::BLOB("data"));
	auto expr = TranslateOrWidenFilter(filter, MakeField("col"));
	REQUIRE(expr->ToString() == "true");
}

TEST_CASE("optional filter with null child widens to AlwaysTrue", "[filter_translation]") {
	LegacyOptionalFilter filter(nullptr);
	auto expr = TranslateOrWidenFilter(filter, MakeField("col"));
	REQUIRE(expr->ToString() == "true");
}

TEST_CASE("optional filter recurses into its child", "[filter_translation]") {
	auto child = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(42));
	LegacyOptionalFilter filter(std::move(child));
	auto expr = TranslateOrWidenFilter(filter, MakeField("col"));
	REQUIRE(expr->ToString() == "ref(name=\"col\") == 42");
}

TEST_CASE("AND drops unsupported conjunct, keeps supported", "[filter_translation]") {
	auto supported = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(5));
	auto unsupported = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::BLOB("data"));
	LegacyConjunctionAndFilter filter;
...
TEST_CASE("AND with all unsupported children widens to AlwaysTrue", "[filter_translation]") {
	auto u1 = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::BLOB("a"));
	auto u2 = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::BLOB("b"));
	LegacyConjunctionAndFilter filter;
...
TEST_CASE("AND with two supported children", "[filter_translation]") {
	auto c1 = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_GREATERTHAN, Value::INTEGER(0));
	auto c2 = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_LESSTHAN, Value::INTEGER(100));
	LegacyConjunctionAndFilter filter;
...
TEST_CASE("OR widens when any child is unsupported", "[filter_translation]") {
	auto supported = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(5));
	auto unsupported = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::BLOB("data"));
	LegacyConjunctionOrFilter filter;
	filter.child_filters.push_back(std::move(supported));
	filter.child_filters.push_back(std::move(unsupported));
...
TEST_CASE("OR with all supported children", "[filter_translation]") {
	auto c1 = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(1));
	auto c2 = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(2));
	LegacyConjunctionOrFilter filter;
	filter.child_filters.push_back(std::move(c1));
	filter.child_filters.push_back(std::move(c2));
...
TEST_CASE("nested AND(OR(supported, unsupported), supported) drops the widened OR", "[filter_translation]") {
	auto or_ok = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(1));
	auto or_bad = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::BLOB("x"));
	auto or_filter = make_uniq<LegacyConjunctionOrFilter>();
	or_filter->child_filters.push_back(std::move(or_ok));
	or_filter->child_filters.push_back(std::move(or_bad));

	auto and_ok = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_GREATERTHAN, Value::INTEGER(10));
	LegacyConjunctionAndFilter filter;
...
TEST_CASE("nested OR(AND(unsupported, supported), supported) simplifies AND and keeps both OR branches",
          "[filter_translation]") {
	auto and_bad = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::BLOB("x"));
	auto and_ok = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_GREATERTHAN, Value::INTEGER(10));
	auto and_filter = make_uniq<LegacyConjunctionAndFilter>();
	and_filter->child_filters.push_back(std::move(and_bad));
	and_filter->child_filters.push_back(std::move(and_ok));

	auto or_ok = make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(5));
	LegacyConjunctionOrFilter filter;
...
TEST_CASE("single column resolves from schema", "[filter_translation]") {
	...
	PushFilter(filter_set, 0, make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(5)),
	           LogicalType::INTEGER);
...
TEST_CASE("multiple columns produce AND", "[filter_translation]") {
	...
	PushFilter(filter_set, 0, make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(5)),
	           LogicalType::INTEGER);
	PushFilter(filter_set, 1, make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_GREATERTHAN, Value("abc")),
	           LogicalType::VARCHAR);
...
TEST_CASE("unsupported column dropped, supported kept", "[filter_translation]") {
	TableFilterSet filter_set;
	PushFilter(filter_set, 0, make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::BLOB("bad")),
	           LogicalType::BLOB);
	PushFilter(filter_set, 1, make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(42)),
	           LogicalType::INTEGER);
...
TEST_CASE("IS NOT NULL resolves from schema", "[filter_translation]") {
	TableFilterSet filter_set;
	PushFilter(filter_set, 0, make_uniq<LegacyIsNotNullFilter>(), LogicalType::INTEGER);
...
TEST_CASE("unsupported comparison op widens to AlwaysTrue", "[filter_translation]") {
	...
	LegacyConstantFilter filter(ExpressionType::COMPARE_DISTINCT_FROM, Value::INTEGER(7));
	auto expr = TranslateOrWidenFilter(filter, MakeField("col"));
	REQUIRE(expr->ToString() == "true");
}
...
TEST_CASE("out-of-bounds column_idx is skipped, not UB", "[filter_translation]") {
	...
	PushFilter(filter_set, 0, make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(5)),
	           LogicalType::INTEGER);
	PushFilter(filter_set, 5, make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(9)),
	           LogicalType::INTEGER);
...
TEST_CASE("all out-of-bounds column_idx widens to AlwaysTrue", "[filter_translation]") {
	...
	TableFilterSet filter_set;
	PushFilter(filter_set, 3, make_uniq<LegacyConstantFilter>(ExpressionType::COMPARE_EQUAL, Value::INTEGER(1)),
	           LogicalType::INTEGER);
...
TEST_CASE("set-level IN resolves column from schema", "[filter_translation]") {
	TableFilterSet filter_set;
	vector<Value> values = {Value::INTEGER(1), Value::INTEGER(2), Value::INTEGER(3)};
	PushFilter(filter_set, 1, make_uniq<LegacyInFilter>(std::move(values)), LogicalType::INTEGER);
...
TEST_CASE("set-level OR resolves column from schema", "[filter_translation]") {
	...
	PushFilter(filter_set, 0, std::move(or_filter), LogicalType::INTEGER);
...
TEST_CASE("set-level nested AND resolves column from schema", "[filter_translation]") {
	...
	PushFilter(filter_set, 1, std::move(and_filter), LogicalType::INTEGER);
